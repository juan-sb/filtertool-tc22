<script>
  ...
  import {
    computeStep, zpkGainFromBa, responseDuration, pickTimeUnit, scaleTimeResponse, STEP_Y_LABEL,
  } from '../../lib/time-response.js'
  import BodePlot from '../BodePlot.svelte'

  function stepOf(fr, pts, tEnd) {
    if (!fr) return null
    const k = zpkGainFromBa(fr.num, fr.den)
    if (k == null) return null
    return computeStep(fr.zeros, fr.poles, k, pts, tEnd)
  }
  ...
  $: main = scaleTimeResponse(stepOf($filterResult, $bodePoints, tEnd), timeUnit.scale)
  ...
  $: traces = [
    ...
    ...$comparisons.flatMap(c => {
      const tr = scaleTimeResponse(stepOf(c.filterResult, $bodePoints, tEnd), timeUnit.scale)
      if (!tr) return []
      return [{
        x: tr.time,
        y: tr.value,
...
{#if !$filterResult}
  <div class="empty"><p>Design a filter to see its step response.</p></div>
{:else}
  <div class="time-tab">
    {#if !main}
      <p class="compute-warn">Could not compute step response for this transfer function.</p>
    {/if}
    <BodePlot
      {traces}
